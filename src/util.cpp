...
const char * const BITCOIN_CONF_FILENAME = "megabitcoin.conf";
const char * const BITCOIN_PID_FILENAME = "megabitcoin.pid";
...
#else
    const char* pszModule = "megabitcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Megabitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Megabitcoin";
#else
    // Unix
    return pathRet / ".megabitcoin";
#endif
#endif
}
